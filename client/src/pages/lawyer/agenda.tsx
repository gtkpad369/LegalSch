    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50">
      ...
      <main className="flex-1 p-4 md:p-6 md:ml-64 pb-16 md:pb-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 mb-6">
          <h2 className="text-lg font-bold text-slate-800 mb-2">Link de Agendamento Público</h2>
          ...
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <Button variant="outline" onClick={() => {
              navigator.clipboard.writeText(`https://jurisagenda.com.br/book/rafael-silva`);
              toast({
                title: "Link copiado",
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/agenda?view=list" className="text-primary text-sm hover:underline">
              Ver todos
            </Link>
          </div>
              ...
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Cliente/Título</th>
                  ...
                  {appointments.slice(0, 5).map((appointment) => (
                    <tr key={appointment.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                        <div className="font-medium">{format(parseISO(appointment.date.toString()), 'dd/MM/yyyy')}</div>
                        <div className="text-slate-500">{appointment.startTime} - {appointment.endTime}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                        {appointment.isPublic ? (
                          <div>
                            <div className="font-medium">{appointment.clientName}</div>
                            <div className="text-slate-500 truncate max-w-[200px]">{appointment.appointmentReason}</div>
                          </div>
                        ) : (
                          <div>
                            <div className="font-medium">{appointment.title}</div>
                            <div className="text-slate-500 truncate max-w-[200px]">{appointment.description}</div>
                          </div>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link href={`/appointments/${appointment.id}`} className="text-primary hover:text-primary-dark mr-4">
                          Detalhes
                        </Link>
                      </td>
                    </tr>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          Defina se este é um horário disponível para agendamento de clientes.
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input {...field} placeholder="Ex: Audiência, Reunião, etc." />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
